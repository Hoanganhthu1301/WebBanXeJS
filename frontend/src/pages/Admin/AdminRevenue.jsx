    ...
    <div className="admin-revenue-page">
      <div className="revenue-topbar">
        <div>
          ...
          <p>
            Thống kê doanh thu theo ngày, tháng, năm với biểu đồ trực quan
          </p>
        </div>
      ...
      <div className="revenue-summary-grid">
        <div className="revenue-summary-card blue">
          <span>Doanh thu tiền cọc</span>
          <h2>{formatMoney(summary?.depositRevenue)}</h2>
        </div>

        <div className="revenue-summary-card amber">
          <span>Doanh thu thanh toán đủ</span>
          <h2>{formatMoney(summary?.fullPaymentRevenue)}</h2>
        </div>

        <div className="revenue-summary-card dark">
          <span>Tổng doanh thu</span>
          <h2>{formatMoney(summary?.totalRevenue)}</h2>
        </div>

        <div className="revenue-summary-card green">
          <span>Tổng giảm giá</span>
          <h2>{formatMoney(getDiscountTotal(rows))}</h2>
        </div>
      </div>
          ...
          <div className="chart-box">
            <ResponsiveContainer width="100%" height={320}>
              <PieChart>
                ...
                >
                  {charts.pieRevenue.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
      ...
      {type === "month" && (
        <div className="chart-card full-width">
          <div className="chart-card-header">
            <h3>
              Xu hướng doanh thu theo ngày trong tháng {month}/{year}
            </h3>
      ...
      <div className="revenue-table-card">
        <div className="chart-card-header">
          ...
          <p>Danh sách các đơn phát sinh tiền trong kỳ</p>
        </div>
          ...
          <table className="revenue-table">
            <thead>
              <tr>
                ...
                <th>Tiền cọc</th>
                <th>Tiền còn lại</th>
                <th>Thu cọc trong kỳ</th>
                <th>Thu phần còn lại</th>
                <th>Ngày cọc</th>
                <th>Ngày thanh toán đủ</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                rows.map((item) => (
                  <tr key={item._id || item.depositId}>
                    ...
                    <td>
                      {hasVoucher(item)
                        ? item.promotionTitle || "Ưu đãi áp dụng"
                        : "-"}
                    </td>
                    <td
                      style={
                    ...
                    <td>
                      {item.fullyPaidAt
                        ? new Date(item.fullyPaidAt).toLocaleString("vi-VN")
                        : "-"}
                    </td>
                  </tr>
                ))
