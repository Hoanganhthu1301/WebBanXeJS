...
export default function CarsPage() {
  ...
  const handleToggleFavorite = async (carId) => {
    ...
    try {
      await axios.post(`https://webbanxe-backend-stx9.onrender.com/api/favorites/toggle/${carId}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
      await fetchFavorites();
    } catch (error) {
  ...
  const brands = useMemo(() => [...new Set(cars.map((car) => car.brand).filter(Boolean))], [cars]);
  const categories = useMemo(() => [...new Set(cars.map((car) => car.category).filter(Boolean))], [cars]);

  const filteredCars = useMemo(() => {
    return cars.filter((car) => {
      const matchesKeyword = !keyword || car.name?.toLowerCase().includes(keyword.toLowerCase()) || car.brand?.toLowerCase().includes(keyword.toLowerCase());
      ...
      return matchesKeyword && matchesBrand && matchesCategory && matchesMinPrice && matchesMaxPrice;
    });
  ...
  const resetFilters = () => {
    setKeyword(""); setSelectedBrand(""); setSelectedCategory("");
  ...
  return (
    <div className="cars-page">
      ...
      <section className="cars-hero">
        ...
        <img src="https://images.unsplash.com/photo-1494976388531-d1058494cdd8?auto=format&fit=crop&w=1600&q=80" alt="banner" className="cars-hero-bg" />
      ...
      <section className="cars-filter-section">
        <div className="cars-filter-box">
          <div className="cars-filter-top">
            ...
            <button className="reset-btn" onClick={resetFilters}>{t('cars_filter_reset')}</button>
          </div>
          <div className="cars-filter-grid">
            <input type="text" placeholder={t('filter_name_placeholder')} value={keyword} onChange={(e) => setKeyword(e.target.value)} />
            ...
            <input type="number" placeholder={t('filter_price_from')} value={minPrice} onChange={(e) => setMinPrice(e.target.value)} />
            <input type="number" placeholder={t('filter_price_to')} value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} />
          </div>
        </div>
      </section>
      ...
      <section className="cars-list-section">
        <div className="cars-list-header">
          <p>
            ...
            {filteredCars.length > 0 && totalPages > 1 ? ` • ${t('cars_list_page', { current: currentPage, total: totalPages })}` : ""}
          </p>
        </div>
          ...
          <>
                ...
                paginatedCars.map((car) => (
                  <div className="cars-card" key={car._id}>
                    <div className="cars-card-image-wrap">
                      ...
                      <button type="button" onClick={() => handleToggleFavorite(car._id)} className={`cars-favorite-btn ${isFavorite(car._id) ? "active" : ""}`} title={t('favorite_title')}>
                    ...
                    <div className="cars-card-content">
                      ...
                      <div className="cars-card-meta">
                        <p><span>{t('label_brand')}</span><strong>{car.brand}</strong></p>
                        <p><span>{t('label_category')}</span><strong>{car.category}</strong></p>
                        <p><span>{t('label_year')}</span><strong>{car.year}</strong></p>
                      </div>
                      <p className="cars-card-price">{Number(car.price || 0).toLocaleString("vi-VN")}đ</p>
                      
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginTop: "16px" }}>
                        <Link to={`/cars/${car._id}`} className="cars-detail-link-new" style={{ height: "52px", borderRadius: "12px", background: "#fff", color: "#111", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none" }}>
                          {t('btn_view_detail')}
                        </Link>
                        <button type="button" onClick={() => handleCompare(car._id)} style={{ height: "52px", border: "1px solid rgba(255,255,255,0.14)", borderRadius: "12px", background: "rgba(255,255,255,0.04)", color: "#fff", fontWeight: 700, cursor: "pointer" }}>
                          {t('btn_compare')}
                        </button>
                      </div>
                    </div>
            ...
            {filteredCars.length > 0 && totalPages > 1 && (
              <div className="cars-pagination">
                <button className="cars-page-btn" onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))} disabled={currentPage === 1}>
                  {t('pagination_prev')}
                </button>
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <button key={page} className={`cars-page-number ${currentPage === page ? "active" : ""}`} onClick={() => setCurrentPage(page)}>{page}</button>
                ))}
                <button className="cars-page-btn" onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages}>
                  {t('pagination_next')}
                </button>
              </div>
            )}
